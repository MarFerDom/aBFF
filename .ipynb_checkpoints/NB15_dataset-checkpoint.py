...
def main(pcap, filename):   
    
    global DS
    
    # load the CSVs from a specific pcap file
    print("loading argus.csv..")
    df = pd.read_csv(filename + "output/argus.csv")                                             # dataset Argus

    print("loading conn.log..")
    zconn = pd.read_csv(filename + "output/conn.log", sep='\t', skiprows = [0, 1, 2, 3, 4, 5, 7]) # dataset Zeek Conn
    ...
    zhttp = pd.read_csv(filename + "output/http.log", sep='\t', skiprows = [0, 1, 2, 3, 4, 5, 7]) # dataset Zeek http
    ...
    zftp = pd.read_csv(filename + "output/ftp.log", sep='\t', skiprows = [0, 1, 2, 3, 4, 5, 7])   # dataset Zeek ftp
    ...
    if zhttp.columns.isin(['ts','id.orig_h','id.orig_p','id.resp_h','id.resp_p']).any():
        zhttp.columns = ['StartTime', 'uid', 'SrcAddr', 'Sport', 'DstAddr','Dport','trans_depth', 'method', 'host', 'uri',
                         'referrer', 'version','user_agent', 'origin', 'request_body_len', 'response_body_len','status_code',
                         'status_msg', 'info_code', 'info_msg', 'tags','username', 'password', 'proxied', 'orig_fuids',
    ...
    zhttp['service'] = 'http'
    zhttp['Proto'] = 6
    zhttp = zhttp.astype({'StartTime':'float','SrcAddr':'string', 'Sport':'int32','DstAddr':'string','Dport':'int32','Proto':'int32',
                          'service':'string','trans_depth':'int32','response_body_len':'int32','method':'string'})
    ...
    if zftp.columns.isin(['id.orig_h','id.orig_p','id.resp_h','id.resp_p']).any():
        zftp.columns = ['StartTime', 'uid', 'SrcAddr', 'Sport', 'DstAddr','Dport','user','password','command','arg','mime_type','file_size',
                        'reply_code','reply_msg','data_channel.passive','data_channel.orig_h','data_channel.resp_h','data_channel.resp_p','fuid']
    ...
    print(DS.head(4))
    print(DS.shape)
    DS.to_csv(filename + 'dataset/' + pcap + '_NB15.csv', index=None, header=True)
    
if __name__ == "__main__":
    pcap = "unknown"
    filename = "./"
    if len(sys.argv) == 0:
        print("Usage: NB15_dataset <PCAP_FILE> <PATH_TO_CSV>")
        sys.exit()
    if len(sys.argv)>1:
        pcap = sys.argv[1]
    if len(sys.argv)>2:
