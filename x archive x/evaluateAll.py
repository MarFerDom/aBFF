...
def runEvaluation(pNum, maxNumFiles, dNum=1, scanOnly=False, scan=True, no_overwrite=True):
    ...
    #----------------------#
    # laod target from given pNum
    tName = myFunc.getDSName(pNum, dNum, scanOnly, scan)
    X, y = myFunc.setTarget(myFunc.loadDataset(pNum, maxNumFiles, dNum), pNum, scanOnly, scan, pNum)
    
    # for every dataset, 
    for pcap in myFunc.pcapType.keys():
        if pcap == pNum:
            continue
